...
def Generate(size,data,P):
    ...
    for i in range(size-1):
        b = randint(0,size-1)
        if check[b] == 1 or P[start][b]+P[b][start] < const:
            check_inner = []
            for j in range(len(check)):
                if check[j] == 0:
                    check_inner.append(j)
            while (check[b] == 1 or P[start][b]+P[b][start] < const) and len(check_inner) > 0:
                b = randint(0,size-1)
                if b in check_inner :
...
def Update(P,better,worse,n,pop_size):
    ...
    for i in range(len(P)):
        temp = P[i][:i]
        temp1 = [P[x][i] for x in range(i,len(P))]
        temp1 = sum(list(temp) + list(temp1))
        P[i][:i] = np.array(P[i][:i])/float(temp1)
        for x in range(i,len(P)):
            P[x][i] = np.array(P[x][i])/float(temp1)
...
def TSP_Cga(size,data,pop_size,expect,limit,shared_pop,result,check):
    # for graph
    value = []
    ...
    while(True):
        ...
        if num_fitness%(10) == 0:
            for idx,n in enumerate(shared_pop):
                n = list(np.array(n)- np.array(prev_recieve[idx])+np.array(P[idx]))
...
if __name__ == '__main__':
    ...
    result = [0,12,1,14,8,4,6,2,11,13,9,7,5,3,10]
        ...
        # shared_pop.append([0.5]*len(data))
    # print(data)
    # print(Tour_Length(result,data))
    best_value = sys.maxsize
    best_array = []
    ...
    for i in range(10):
        ...
        processes = [mp.Process(target=TSP_Cga, args=(len(data),data,pop_size,expect,limit,shared_pop,result1,check)),mp.Process(target=TSP_Cga, args=(len(data),data,pop_size,expect,limit,shared_pop,result2,check))]
...
##        cal_fitness = cal_fitness+ min(result1['num_fitness'],result2['num_fitness'])
    ...
    # start = time.time()
    # i = mp.Value("i",1)
    # processes = [mp.Process(target=cube, args=(i,)) for j in range(1,7)]
    # [process.start() for process in processes]
