...
##    print(1.0/size)
    ...
    for i in range(size-1):
        b = randint(0,size-1)
        if check[b] == 1 or max(P[start][b],P[b][start]) < 0.8/size:
            check_inner = []
            for j in range(len(check)):
                if check[j] == 0:
                    check_inner.append(j)
            while (check[b] == 1 or max(P[start][b],P[b][start]) < 0.8/size) and len(check_inner) > 0:
                b = randint(0,size-1)
                if b in check_inner :
            ...
            start = b
##        temp = P[start][:start]
##        temp1 = [P[x][start] for x in range(start,size)]
##        temp1[0] = -1000
##        temp = list(temp) + list(temp1)
##        max_now = max(temp)
##        index = temp.index(max_now)
##        while(check[index] == 1):
##            temp[index] = -1
##            max_now = max(temp)
##            index = temp.index(max_now)
